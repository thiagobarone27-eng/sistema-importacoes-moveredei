...
export function montarImportacaoExpandida(row: any, config: ConfigEficienciaLimiares) {
  const valoresBrutos = {
    ...
    desconsolidacao: row.desconsolidacao,
    taxaLiberacao: row.taxaLiberacao,
    docFeeOrigin: row.docFeeOrigin,
    customsOrigin: row.customsOrigin,
    pickUp: row.pickUp,
    palletFee: row.palletFee,
    exportLicense: row.exportLicense,
    devolucaoVazio: row.devolucaoVazio,
    lavagem: row.lavagem,
    fichaEmergencia: row.fichaEmergencia,
    impostosFederais: row.impostosFederais,
    afrmm: row.afrmm,
