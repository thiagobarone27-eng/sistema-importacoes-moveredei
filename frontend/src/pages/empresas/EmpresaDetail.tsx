...
import { ArrowLeft, Boxes, DollarSign, Gauge, Package, Percent } from "lucide-react";
...
export function EmpresaDetail() {
  ...
  const analise = useAsync(() => empresasApi.analise(Number(id)), [id]);

  if (analise.loading) return <LoadingState label="Carregando análise da empresa..." />;
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link to="/empresas" className="mb-2 inline-flex items-center gap-1.5 text-sm text-ink-500 hover:text-brand-700">
          <ArrowLeft size={15} /> Voltar para empresas
        </Link>
        <PageHeader
          title={dados.empresa.nome}
          subtitle={`${formatNumber(dados.numeroImportacoes)} importações · ${dados.produtos.length} produto(s)`}
        />
      </div>
      ...
      <Card>
        <CardHeader title="Produtos importados" />
        <CardBody>
          <div className="flex flex-wrap gap-2">
            {dados.produtos.map((p) => (
              <span key={p} className="rounded-full bg-ink-100 px-3 py-1 text-xs font-medium text-ink-600">
                {p}
              </span>
            ))}
          </div>
      ...
      <Card>
        <CardHeader title="Evolução do valor total" subtitle="Ao longo das importações desta empresa" />
        <CardBody>
          {evolucao.length === 0 ? (
